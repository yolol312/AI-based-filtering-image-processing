...
def parse_filename(filename):
    match = re.match(r'person_(\d+)_frame_(\d+)_gender_(\w+)_age_(\w+)_color_(\w+)_upclothes_(\w+)_downclothes_(\w+)\.jpg', filename)
    if match:
        ...
        upclothes = match.group(6).lower()
        downclothes = match.group(7).lower()
        return person_id, frame, gender, age, color, upclothes, downclothes
    return None
...
def gather_info_from_files(directory):
    ...
    for file in os.listdir(directory):
        if file.endswith('.jpg'):
            ...
            if parsed_info:
                person_id, frame, gender, age, color, upclothes, downclothes = parsed_info
                if person_id not in info_dict:
                    info_dict[person_id] = []
                info_dict[person_id].append((frame, gender, age, color, upclothes, downclothes)) ##수정
...
def save_info_to_txt(info_dict, output_file, filter_gender, filter_age, filter_color, filter_upclothes, filter_downclothes): 
    filtered_persons = []
    with open(output_file, 'w') as f:
        for person_id in sorted(info_dict.keys()):
            frames = [frame for frame, gender, age, color, upclothes, downclothes in info_dict[person_id]]            
            genders = [gender for frame, gender, age, color, upclothes, downclothes in info_dict[person_id]]          
            ages = [age for frame, gender, age, color, upclothes, downclothes in info_dict[person_id]]             
            colors = [color for frame, gender, age, color, upclothes, downclothes in info_dict[person_id]]            
            upclothes_types = [upclothes for frame, gender, age, color, upclothes, downclothes in info_dict[person_id]]    
            downclothes_types = [downclothes for frame, gender, age, color, upclothes, downclothes in info_dict[person_id]] 
            ...
            print(f"Person {person_id} - Color Counts: {color_counts}, UpClothes Counts: {upclothes_counts}, DownClothes Counts: {downclothes_counts}")
            print(f"Person {person_id} - Most common color: {most_common_color}, Most common upclothes: {most_common_upclothes}, Most common downclothes: {most_common_downclothes}")   
...
def save_best_faces(image_files, output_folder, info_dict, filtered_persons):
    ...
    for person_id, filepath in image_files:
        ...
        try:
            ...
            if not parsed_info:
                continue
            _, _, image_gender, image_age, image_color, image_upclothes, image_downclothes = parsed_info
            
            stored_gender = get_most_common([gender for _, gender, _, _, _, _ in info_dict[person_id]])
            stored_age = get_most_common([age for _, _, age, _, _, _ in info_dict[person_id]])
            stored_color = get_most_common([color for _, _, _, color, _, _ in info_dict[person_id]])
            stored_upclothes = get_most_common([upclothes for _, _, _, _, upclothes, _ in info_dict[person_id]])
            stored_downclothes = get_most_common([downclothes for _, _, _, _, _, downclothes in info_dict[person_id]])

            if image_gender != stored_gender or image_age != stored_age or image_color != stored_color or image_upclothes != stored_upclothes or image_downclothes != stored_downclothes:  ##수정    
                continue
...
if __name__ == "__main__":
    try:
        ...
        user_id = sys.argv[2]# "WS_TEST2"  #sys.argv[2]
        filter_id = sys.argv[3]#  "47"   #sys.argv[3]
        filter_gender = sys.argv[4].lower()# "남성"    #sys.argv[4].lower()
        filter_age = sys.argv[5].lower()# "none"   #sys.argv[5].lower()
        filter_color = sys.argv[6].lower()# "none" #sys.argv[6].lower()
        filter_upclothes = sys.argv[7].lower()# "none" #sys.argv[7].lower()
        filter_downclothes = sys.argv[8].lower()# "none"   #sys.argv[8].lower()
            ...
            if '_face' in video_folder and video_name in video_folder:  # video_name과 일치하는 폴더만 처리
                ...
                if os.path.isdir(folder_path):
                    try:
                        ...
                        filtered_persons = save_info_to_txt(info_dict, output_file, filter_gender, filter_age, filter_color, filter_upclothes, filter_downclothes) # 필터링을 해당 메서드에 넣음
