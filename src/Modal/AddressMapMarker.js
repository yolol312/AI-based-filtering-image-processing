...
function AddressMapMarker({ onBack, onClose }) {
  ...
  const fetchMapCameraInfo = async () => {
    ...
    try {
      ...
      const response = await fetch(`${Server_IP}/update_maps?user_id=${userId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.log('받아온 map_camera_info:', data.map_camera_info); // 콘솔에 데이터 출력
      // 가져온 데이터를 DataContext에 업데이트합니다.
      setData({ mapCameraInfo: data.map_camera_info });
    } catch (error) {
  ...
  return (
    <div className="address-map-marker">
      ...
      />
      <button className="address-submit-button" onClick={handleAddressSearch}> 주소 검색</button>
