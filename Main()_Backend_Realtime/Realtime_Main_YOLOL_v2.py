...
def realtime_upload_image():
    ...
    if image_count % 6 == 0 and image_count % 30 != 0:
        # 실시간 트래킹 처리
    ...
    '''
    if image_count % 6 == 0 and image_count % 60 != 0:
        # 실시간 트래킹 처리
        person_detections = detect_objects(yolo_model, img, device)
        ...
        print(f"Received and processed image from cam {cam_name} with shape: {img.shape} as {filename} / predictions : {predictions}")
        return jsonify({"message": "Image received and processed", "frame": image_base64, "person_data" : predictions, "person_ids" : track_ids}), 200
    '''
    if image_count % 30 == 0 and image_count != 0:
        # 결과 이미지를 Base64로 인코딩하여 클라이언트에 반환
        _, buffer = cv2.imencode('.jpg', img)
        image_base64 = base64.b64encode(buffer).decode('utf-8')
