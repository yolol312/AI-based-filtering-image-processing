...
function AddressMapMarker({ onBack, onClose }) {
  ...
  const fetchMapCameraInfo = async () => {
    ...
    try {
      ...
      const response = await fetch(`${Server_IP}/update_pro_video?user_id=${userId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.log('받아온 map_camera_provideo_info:', data1.map_camera_provideo_info); // 콘솔에 데이터 출력
  ...
  return (
    <div className="address-map-marker">
      <h2>주소 업로드</h2>
      <input id="address-input" type="text" placeholder="여기에 주소를 입력하세요" />
      <button className="address-submit-button" onClick={handleAddressSearch}>
        {' '}
        주소 검색
      </button>
